...
public class Serie{
    ...
    public static Serie[] bolha(Serie[] array) {
		for (int i = (tam1 - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
				numComparacoes += 3;
				if (array[j].getNumTemporadas() > array[j + 1].getNumTemporadas() || (array[j].getNumTemporadas() == array[j + 1].getNumTemporadas() && array[j].getNome().compareTo(array[j+1].getNome()) > 0)) {
                    swap(array, j, j+1);
				}
			}
		}
	...
	public static void main(String[] args)throws ParseException{
		...
		Serie[] seriesOrdenadas = bolha(serie);
		...
		Arq.openWrite("716274_bolha.txt");
