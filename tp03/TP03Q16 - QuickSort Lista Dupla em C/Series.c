...
CelulaDupla* novaCelulaDupla(Series* elemento) {
...
void quicksort(int esq, int dir) {
    int i = esq, j = dir;
    CelulaDupla* pivo = getCelula((dir+esq)/2);
    while (i <= j) {
        while ((strcmp(getCelula(i)->elemento->paisOriginal, pivo->elemento->paisOriginal) < 0) || (strcmp(getCelula(i)->elemento->paisOriginal, pivo->elemento->paisOriginal) == 0 && strcmp(getCelula(i)->elemento->nome, pivo->elemento->nome) < 0)) i++;
        while ((strcmp(getCelula(j)->elemento->paisOriginal, pivo->elemento->paisOriginal) > 0) || (strcmp(getCelula(j)->elemento->paisOriginal, pivo->elemento->paisOriginal) == 0 && strcmp(getCelula(j)->elemento->nome, pivo->elemento->nome) > 0)) j--;
        if (i <= j) {
            Series* tmp = getCelula(i)->elemento;
            getCelula(i)->elemento = getCelula(j)->elemento;
            getCelula(j)->elemento = tmp; 
            i++;
            j--;
        }
    }
