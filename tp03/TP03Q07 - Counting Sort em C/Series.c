...
int getMaior(Series** array) {
    Series* maior = array[0];

    for (int i = 0; i < num; i++) {
        numComparacoes++;
        if(getNumTemporadas(maior) < getNumTemporadas(array[i])){
            maior = array[i];
        }
    }
    return getNumTemporadas(maior);
}

Series** countingsort(Series **array) {
    //Array para contar o numero de ocorrencias de cada elemento
    int tamCount = getMaior(array) + 1;
    int count[tamCount];
    Series** ordenado = (Series **)calloc(1000, sizeof(Series *));

    //Inicializar cada posicao do array de contagem 
    for (int i = 0; i < tamCount; count[i] = 0, i++);

    //Agora, o count[i] contem o numero de elemento iguais a i
    for (int i = 0; i < num; count[getNumTemporadas(array[i])]++, i++);
    ...
    for(int i = num-1; i >= 0; i--){
        ordenado[count[getNumTemporadas(array[i])]-1] = array[i];
        numMovimentacoes++;
        count[getNumTemporadas(array[i])]--;
    }

    //Copiando para o array original
    for(int i = 0; i < num; array[i] = ordenado[i], i++);

    //Faz o desmpate pelo nome caso o numero de temporadas sejao mesmo
    for (int i = (num - 1); i > 0; i--) {
		for (int j = 0; j < i; j++) {
            numComparacoes += 2;
			if (getNumTemporadas(array[j]) == getNumTemporadas(array[j + 1]) && strcmp(getNome(array[j]), getNome(array[j+1])) > 0) {
                Series* tmp = array[j];
                array[j] = array[j+1];
                array[j+1] = tmp;
                numMovimentacoes += 3;
...
int main() {
    ...
    start = clock();
    series = countingsort(series);
    stop = clock();
    double tempTotal = (double)(stop - start);
    FILE* arquivo = fopen("716274_countingsort.txt", "wt");
