...
public class Serie{
    ...
    public static Serie[] mergesort(Serie[] array, int esq, int dir) {
        if (esq < dir){
           int meio = (esq + dir) / 2;
           mergesort(array, esq, meio);
           mergesort(array, meio + 1, dir);
           array = intercalar(array, esq, meio, dir);
        }
        return array;
    }
  
    public static Serie[] intercalar(Serie[] array, int esq, int meio, int dir){
        int n1, n2, i, j, k;
  
		Serie sentinela = new Serie();	
		sentinela.setNumEpisodios(0x7FFFFFFF);
        ...
        for(i = j = 0, k = esq; k <= dir ; k++){
			numComparacoes += 3;
            array[k] = (a1[i].getNumEpisodios() < a2[j].getNumEpisodios() || (a1[i].getNumEpisodios() == a2[j].getNumEpisodios() && a1[i].getNome().compareTo(a2[j].getNome()) < 0)) ? a1[i++] : a2[j++];
        }
        return array;
    }
	...
	public static void main(String[] args)throws ParseException{
		...
		Serie[] seriesOrdenadas = mergesort(serie, 0, tam1-1);
		...
		Arq.openWrite("716274_mergesort.txt");
