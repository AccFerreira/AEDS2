...
public class Serie{
    ...
    public Serie(String nome, String formato, String duracao, String paisDeOrigem, String idiomaOriginal, String emissoraOriginal, String transmissaoOriginal, int numTemporadas, int numEpisodios, int numDoidao){
		super();
		this.nome = nome;
	...
	public Serie cloneSerie(){
		...
		cloneSerie.numDoidao = numDoidao;
		return cloneSerie;
	}
    ...
    public static Serie leituraHtml(String linhasPubIn) throws IOException {
		...
		s.numDoidao  = s.numEpisodios*1000 + s.numTemporadas;
       	//s.imprimir();  
	...
	public static Serie[] radixSort(Serie[] serie) {
		int i, exp = 1;
		int[] aux = new int[tam];
		Serie[] temp = new Serie[tam];
		Serie s = new Serie();
	
		for(i = 0; i < tam; i++) {
			if(serie[i].getNumDoidao() > s.getNumDoidao()) {
				s = serie[i];
			}
		}
	
		while((s.getNumDoidao()/exp) > 0) {
			for (i = 0; i < tam; i++) {
				aux[i] = 0;
			}
			for(i = 0; i < tam; i++) {
				aux[(serie[i].getNumDoidao() / exp) % 10]++;
			}
			for(i = 1; i < tam; i++) {
				aux[i] += aux[i-1];
			}
			for(i = (tam - 1); i >= 0; i--) {
				temp[--aux[(serie[i].getNumDoidao() / exp) % 10]] = serie[i];
			}
			for(i = 0; i < tam; i++) {
				serie[i] = temp[i];
			}
    ...
    public static String[] leituraPubIn(){
        ...
        while(conferirFim(linhas[tam]) == false){ /* LEITURA DO ARQUIVO ENQUANTO ESSE NÃO CHEGAR NO FIM */
			tam++;
        	linhas[tam] = MyIO.readLine();
		}
		return linhas;
	...
	public static void main(String[] args)throws ParseException{
		...
		Serie[] seriesOrdenadas = radixSort(serie);
		...
		Arq.openWrite("716274_radixsort.txt");
