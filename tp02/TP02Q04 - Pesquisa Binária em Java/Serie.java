...
public class Serie{
	...
	public static Serie[] ordenarSeries(Serie[] series){
		Serie tmp = new Serie();
    ...
    public static boolean pesquisaBinaria(Serie[] series, String linha){
      boolean resp = false;
      int dir = tam1, esq = 0, meio;
      while (esq <= dir){
         meio = (esq + dir) / 2;
         if(linha.compareTo(series[meio].getNome()) == 0){
         	numComparações++;
            resp = true;
            esq = dir + 1;
         } else if (linha.compareTo(series[meio].getNome()) > 0) {
         	numComparações = numComparações + 2;;
            esq = meio + 1;
         } else {
         	numComparações = numComparações + 2;
            dir = meio - 1;
	...
	public static void main(String[] args)throws ParseException{
		...
		String[] linhasPesquisar = leituraPubIn(2); 
		...
		Serie[] seriesOrdenadas = ordenarSeries(serie);
		i = 0;
		while(i < tam2){
			pesquisaBinaria(seriesOrdenadas, linhasPesquisar[i]);
			i++;
		}
		
		long tempoTotal = System.currentTimeMillis() - tempoInicial;
		Arq.openWrite("716274_binaria.txt");
		Arq.println("Matricula: 716274	" + "Tempo:" + tempoTotal + "	" + "Comparações:" + numComparações);
		Arq.close();
	}
}
