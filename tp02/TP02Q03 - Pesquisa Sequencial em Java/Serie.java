...
public class Serie{
	...
	public static boolean pesquisaSequencial(Serie[] series, String linha){
    	boolean resp = false;
      	for(int j = 0; j < tam1; j++){
      		numComparações++;
         	if(linha.compareTo(series[j].getNome()) == 0){
            	resp = true;
            	j = tam1;
      		}
        }
	...
	public static void main(String[] args)throws ParseException{
		...
		i = 0;
		while(i < tam2){
			pesquisaSequencial(serie, linhasPesquisar[i]);
			i++;
		}
			
		long tempoTotal = System.currentTimeMillis() - tempoInicial;
		Arq.openWrite("716274_sequencial.txt");
